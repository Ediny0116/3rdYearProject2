...
namespace Unity.Multiplayer.Playmode.VirtualProjects.Editor
{
    ...
    class MessagingService
    {
        ...
        readonly List<Action> m_PendingMessages = new List<Action>();
        ...
        public static MainEditorInternalRuntime.MessagingServiceSynchronousCloneAssetImportDelegates GetImportDelegates(MessagingService messagingService) 
        {
            return new MainEditorInternalRuntime.MessagingServiceSynchronousCloneAssetImportDelegates
            {
        ...
        // "--ump-channel-service-on-startup",
        // $"-ump-channel-service-port {ChannelService.GetPort()}",
        ...
        public void HandleUpdate()
        {
            ...
            for (var i = m_Acks.Count - 1; i >= 0; i--)
            {
                ...
                if ((DateTime.UtcNow - ack.SentTimestamp).TotalSeconds > MessagingServiceConstants.MessageTimeoutSeconds)
                {
                    ack.ErrorCallback?.Invoke(
                        $"Timeout reached for client message '{ack.EventType}:{messageId}'");
                    m_Acks.RemoveAt(i);
                }
            }
        }
        ...
        void InternalSend(object message, VirtualProjectIdentifier targetIdentifier, Action onAck, Action<string> onErrorOrTimeout)
        {
            ...
            if ((onAck != null || onErrorOrTimeout != null)
                && message.GetType() != typeof(AckMessage))
            {
                requiresAck = true;
#if UNITY_MP_TOOLS_DEV_LOGMESSAGES
                if (onAck != null)
