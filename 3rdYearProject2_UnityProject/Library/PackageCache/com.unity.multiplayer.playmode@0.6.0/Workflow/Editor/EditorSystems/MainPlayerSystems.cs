...
namespace Unity.Multiplayer.Playmode.Workflow.Editor
{
    class MainPlayerSystems
    {
        ...
        internal static void OptionallyInitializePlayerInitialState(SystemDataStore systemDataStore, VirtualProjectsApi virtualProjectsApi)
        {
            for (var i = 1; i <= 4; i++)
            {
                ...
                if (systemDataStore.TryLoadPlayerJson(i, out var player))
                {
                    if (player.Type != PlayerType.Main
                        ...
                        && virtualProjectsApi.TryGet(player.TypeDependentPlayerInfo.VirtualProjectIdentifier, out var project, out _))
                    {
